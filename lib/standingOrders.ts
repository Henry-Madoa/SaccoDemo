...
export async function runStandingOrders(user: Actor, dateStr: IsoDate = today()): Promise<StandingOrderRunSummary> {
  ...
  // (lib/checkoffBatches.ts); running them here too would double-recover them.
  const orders = await all<{ no: string }>(
    "SELECT no FROM standing_order WHERE running = true AND terminated = false AND salary_based = false ORDER BY no",
  );
