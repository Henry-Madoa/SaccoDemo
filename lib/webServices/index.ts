...
import type { Actor, SessionUser, WebService, WebServiceAccessKey, WebServiceAccessKeyView, WebServiceLogEntry, WebServiceObjectType } from '../types.ts';
 ...
 */
export async function generateAccessKey(userId: number, expiresAt: string | null, user: Actor): Promise<{ key: string; hint: string }> {
  ...
  await run(
    'INSERT INTO web_service_access_key (user_id, key_hash, key_hint, expires_at, created_at, created_by) VALUES (?,?,?,?,?,?)',
    userId, hashKey(key), key.slice(-4), expiresAt || null, now, user.username,
  );
  await audit(user, 'WEB_SERVICE_KEY_ISSUE', 'app_user', userId, { username: u.username, expiresAt });
...
export function statusOfError(e: unknown): { status: number; code: string; message: string } {
  ...
  return { status: 500, code: 'Internal', message: anyE?.message ?? 'Internal error' };
}
