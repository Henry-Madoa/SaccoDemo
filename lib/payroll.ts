...
interface PeriodTxDraft {
  ...
  journalAccountType?: 'GL' | 'SAVINGS' | 'CREDIT' | 'EMPLOYEE';
  companyDeduction?: boolean; loanId?: number | null; imprestNo?: string | null;
}
...
/* ============================================================ SACCO integration (AL port) */

/** The member's FOSA account — AL MemberManagement.GetMemberAccount(Member, "Withdrawable Deposit"). */
export async function getMemberFosaAccount(memberId: number): Promise<{ id: number; account_no: string } | undefined> {
  return one<{ id: number; account_no: string }>(
    `SELECT sa.id, sa.account_no FROM savings_account sa
     JOIN savings_product p ON p.id = sa.product_id
     WHERE sa.member_id = ? AND p.category = 'WITHDRAWABLE DEPOSIT' AND sa.status IN ('ACTIVE','DORMANT')
     ORDER BY sa.id LIMIT 1`, memberId,
  );
}

/**
 * The payroll transaction code a loan product's deductions are booked under — AL's Payroll
 * Transaction Code with Coop Parameter = loan and "Loan Product" = the product, created here on
 * demand as LOAN-<product code>. It credits the Check-off Receivable Clearing account, which the
 * loan repayment on posting then debits, so the clearing nets to nil per period.
 */
async function ensureLoanDeductionCode(product: { id: number; code: string; name: string }): Promise<number> {
  const existing = await one<{ id: number }>('SELECT id FROM payroll_transaction_code WHERE loan_product_id = ?', product.id);
  if (existing) return existing.id;
  const clearing = await one<{ id: number }>('SELECT id FROM gl_account WHERE code = ?', CHANNEL_GL.CHECKOFF);
  if (!clearing) throw new AppError(`Check-off clearing account ${CHANNEL_GL.CHECKOFF} is missing from the chart of accounts`, 'VALIDATION');
  const code = `LOAN-${product.code}`.slice(0, 20);
  const byCode = await one<{ id: number }>('SELECT id FROM payroll_transaction_code WHERE code = ?', code);
  if (byCode) {
    await run('UPDATE payroll_transaction_code SET loan_product_id = ? WHERE id = ?', product.id, byCode.id);
    return byCode.id;
  }
  const info = await run(
    `INSERT INTO payroll_transaction_code (code, name, type, taxable, special_type, gl_account_id, loan_product_id, created_at, created_by)
     VALUES (?,?,?,?,?,?,?,?,?)`,
    code, `${product.name} — loan repayment`, 'DEDUCTION', false, 'LOAN', clearing.id, product.id, new Date().toISOString(), 'SYSTEM',
  );
  return Number(info.lastInsertRowid);
}

/**
 * AL Cod52204009 "Payroll Loan Management".LoanProcessing, for one employee: every disbursed
 * checkoff-recovery loan of the employee's member with a balance becomes a deduction row for the
 * open period — this month's principal + interest from the loan schedule (AL: "Monthly
 * Principal" + balance × rate ⁄ 12), never more than is owed. The rows are refreshed on every
 * run and marked temporary, so a loan repaid or restructured in Credit is picked up next run and
 * nothing rolls into the next period on its own.
 */
async function syncLoanDeductionsFromCredit(periodId: number, period: PayrollPeriod, employeeId: number, memberId: number | null, user: Actor): Promise<void> {
  await run('DELETE FROM employee_payroll_transaction WHERE employee_id = ? AND payroll_period_id = ? AND loan_id IS NOT NULL', employeeId, periodId);
  if (!memberId) return;
  const loans = await all<{ id: number; loan_no: string; product_id: number; product_code: string; product_name: string; installment: number; owed: number }>(
    `SELECT l.id, l.loan_no, l.product_id, p.code AS product_code, p.name AS product_name, l.installment,
            (l.principal_balance + l.interest_balance + l.penalty_balance) AS owed
     FROM loan l JOIN loan_product p ON p.id = l.product_id
     WHERE l.member_id = ? AND l.status = 'DISBURSED' AND l.recovery_mode = 'CHECKOFF'
       AND (l.principal_balance + l.interest_balance + l.penalty_balance) > 0
     ORDER BY l.loan_no`, memberId,
  );
  for (const loan of loans) {
    // This period's installment(s) — what falls due between the period's dates and is unpaid.
    const due = await one<{ amount: number }>(
      `SELECT COALESCE(SUM(GREATEST(principal_due - principal_paid, 0) + GREATEST(interest_due - interest_paid, 0)), 0) AS amount
       FROM loan_schedule WHERE loan_id = ? AND due_date >= ? AND due_date <= ? AND status <> 'PAID'`,
      loan.id, period.start_date, period.end_date,
    );
    let amount = Number(due?.amount ?? 0);
    if (amount <= 0) amount = Number(loan.installment ?? 0);
    amount = Math.min(Math.round(amount), Number(loan.owed));
    if (amount <= 0) continue;
    const codeId = await ensureLoanDeductionCode({ id: loan.product_id, code: loan.product_code, name: loan.product_name });
    await run(
      `INSERT INTO employee_payroll_transaction
         (employee_id, transaction_code_id, payroll_period_id, amount_cents, balance_cents, temporary, notes, loan_id, created_at, created_by)
       VALUES (?,?,?,?,?,true,?,?,?,?)`,
      employeeId, codeId, periodId, amount, Number(loan.owed), `Loan ${loan.loan_no} — from the Credit module`, loan.id,
...
export async function runPayrollForEmployee(periodId: number, employeeId: number, user: Actor): Promise<void> {
  return tx(async () => {
    ...
    // AL OnPayrollLoanManagement: the Credit module's checkoff loans become this run's deductions.
    await syncLoanDeductionsFromCredit(periodId, period, employeeId, emp.member_id, user);
    ...
    for (const row of await linesOfType('DEDUCTION')) {
      ...
      put({ code: code.code, codeId: code.id, name: code.name, type: 'DEDUCTION', group: 'DEDUCTIONS', subOrder: row.loan_id ? 1 : 0, amountCents: amount,
        ...
        journalAccountType: row.loan_id ? 'CREDIT' : ['IMPRECOV', 'IMPCLAIM'].includes(code.code) ? 'EMPLOYEE' : 'GL',
        loanId: row.loan_id ?? null, imprestNo: ['IMPRECOV', 'IMPCLAIM'].includes(code.code) ? row.notes?.match(/(?:Imprest|Staff Claim) (\S+)/)?.[1] ?? null : null });
    ...
    put({ code: 'NPAY', codeId: null, name: PERIOD_CODES.NPAY, type: 'NET', group: 'NET PAY', subOrder: 0, amountCents: netPay,
      ...
      journalAccountType: emp.payment_mode === 'FOSA' ? 'SAVINGS' : 'GL' });
      ...
      await run(
        `INSERT INTO payroll_period_transaction
           (payroll_period_id, employee_id, transaction_code, transaction_code_id, transaction_name, transaction_type,
            ...
            gl_account_id, post_as, post_to_journal, journal_account_type, company_deduction, member_id, loan_id, imprest_no,
        ...
        l.glAccountId ?? null, l.postAs ?? null, l.postToJournal, l.journalAccountType ?? 'GL', !!l.companyDeduction, emp.member_id, l.loanId ?? null, l.imprestNo ?? null,
...
/** The member's check-off loans the payroll run deducts — with what this period would take. */
export const listPayrollLoansForEmployee = (memberId: number): Promise<{ id: number; loan_no: string; product_name: string; installment: Cents; owed: Cents; principal_balance: Cents; interest_balance: Cents }[]> =>
  all(
    `SELECT l.id, l.loan_no, p.name AS product_name, l.installment, l.principal_balance, l.interest_balance,
            (l.principal_balance + l.interest_balance + l.penalty_balance) AS owed
     FROM loan l JOIN loan_product p ON p.id = l.product_id
     WHERE l.member_id = ? AND l.status = 'DISBURSED' AND l.recovery_mode = 'CHECKOFF'
       AND (l.principal_balance + l.interest_balance + l.penalty_balance) > 0
 ...
 *  opened next period. */
/**
 * Rep52204000 "Post Payroll", SACCO side. The period journal above has already credited every
 * employee's net pay to the posting group's Net Pay Payable and every loan deduction to the
 * Check-off Receivable Clearing account. This settles both:
 *   * an employee paid by FOSA (Payroll Salary Card "Payment Mode") has their net pay deposited
 *     into their member's Withdrawable Deposit account, the deposit debiting Net Pay Payable —
 *     the salary is in the FOSA account the moment payroll posts;
 *   * every deduction line that names a loan is repaid in the Credit module through the check-off
 *     channel, which debits the same clearing account the payroll credited, allocating penalty →
 *     interest → principal exactly as a counter or checkoff repayment would.
 * Idempotent per period / employee / loan, so re-posting can never pay or recover twice.
 */
async function postPayrollToSacco(periodId: number, period: PayrollPeriod, journalNo: string, user: Actor): Promise<void> {
  // Guard first, post second: a FOSA-paid employee without an account stops the whole close
  // before anything has moved, rather than half way through.
  const fosaEmployees = await all<{ employee_id: number; employee_no: string; member_id: number | null; net_pay: number; net_pay_payable_account_id: number }>(
    `SELECT e.id AS employee_id, e.employee_no, e.member_id,
            COALESCE((SELECT SUM(l.amount_cents) FROM payroll_period_transaction l WHERE l.payroll_period_id = ? AND l.employee_id = e.id AND l.transaction_code = 'NPAY'), 0) AS net_pay,
            g.net_pay_payable_account_id
     FROM employee e JOIN payroll_posting_group g ON g.id = e.posting_group_id
     WHERE e.payment_mode = 'FOSA' AND EXISTS (SELECT 1 FROM payroll_period_transaction l WHERE l.payroll_period_id = ? AND l.employee_id = e.id)`,
    periodId, periodId,
  );
  const targets: { employee_no: string; account: { id: number; account_no: string }; net_pay: number; contra: number; employee_id: number }[] = [];
  for (const e of fosaEmployees) {
    if (Number(e.net_pay) <= 0) continue;
    if (!e.member_id) throw new AppError(`${e.employee_no} is paid to FOSA but has no SACCO Member No. on the Payroll Salary Card`, 'VALIDATION');
    const account = await getMemberFosaAccount(e.member_id);
    if (!account) throw new AppError(`${e.employee_no}'s member has no Withdrawable Deposit (FOSA) account to pay the salary into`, 'VALIDATION');
    targets.push({ employee_no: e.employee_no, account, net_pay: Number(e.net_pay), contra: e.net_pay_payable_account_id, employee_id: e.employee_id });
  }
  for (const t of targets) {
    await deposit({
      accountId: t.account.id, amount: t.net_pay, channel: 'SYSTEM', contraGlAccount: t.contra, valueDate: period.end_date,
      description: `Net salary — ${period.period_name}`, reference: journalNo, user,
      idempotencyKey: `PAYROLL-${periodId}-NETPAY-${t.employee_id}`,
    });
  }

  const loanLines = await all<{ id: number; loan_id: number; amount_cents: number; employee_no: string; loan_no: string }>(
    `SELECT l.id, l.loan_id, l.amount_cents, e.employee_no, ln.loan_no
     FROM payroll_period_transaction l JOIN employee e ON e.id = l.employee_id JOIN loan ln ON ln.id = l.loan_id
     WHERE l.payroll_period_id = ? AND l.loan_id IS NOT NULL AND l.amount_cents > 0`, periodId,
  );
  for (const l of loanLines) {
    await repay({
      loanId: l.loan_id, amount: Number(l.amount_cents), channel: 'CHECKOFF', valueDate: period.end_date,
      description: `Payroll deduction ${l.employee_no} — ${period.period_name}`, user,
      idempotencyKey: `PAYROLL-${periodId}-LOAN-${l.id}`,
    });
  }
}
...
export async function closePayrollPeriod(id: number, nextOverride: { periodName?: string; startDate?: string; endDate?: string } | null, user: Actor): Promise<{ nextPeriodId: number; journalNo: string; nextPeriodName: string }> {
  return tx(async () => {
    ...
    // AL Rep52204000 "Post Payroll": NPAY to the employee's FOSA account, Coop loan lines to Credit.
    await postPayrollToSacco(id, period, posted.journal_no, user);
 ...
 * per code per period, so an employee cannot carry two Transport Allowances. Loan lines are the
 * exception — the Credit module keys them by loan, and two loans of one product share a code.
 */
export async function assertNoDuplicateLine(employeeId: number, periodId: number, code: Pick<PayrollTransactionCode, 'id' | 'code' | 'name'>): Promise<void> {
  const existing = await one<{ id: number; stopped: boolean; salary_scale_id: number | null }>(
    'SELECT id, stopped, salary_scale_id FROM employee_payroll_transaction WHERE employee_id = ? AND payroll_period_id = ? AND transaction_code_id = ? AND loan_id IS NULL ORDER BY stopped, id LIMIT 1',
