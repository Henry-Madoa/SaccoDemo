...
interface EmpFigures {
  ...
  nssf_no: string | null; shif_no: string | null; bank_account_no: string | null; payment_mode: string; member_id: number | null;
...
const empFigures = (periodId: number, f: ReportFilters) => { const w = employeeWhere(f, 'e'); return all<EmpFigures>(
  `SELECT e.id AS employee_id, e.employee_no, (e.first_name || ' ' || e.last_name) AS name, e.national_id, e.kra_pin, e.nssf_no, e.shif_no,
          e.bank_account_no, e.payment_mode, e.member_id, gd2.name AS department, jg.name AS job_grade,
...
export async function buildPayrollReport(periodId: number, key: PayrollReportKey, filters: ReportFilters = {}): Promise<PayrollReport> {
  ...
  switch (key) {
    ...
    case 'net-pay': {
      // Rep52203528 — Emp. No | Employee Name | Account No. | Net Pay (FOSA account for members paid to FOSA)
      const fosa = new Map((await all<{ member_id: number; account_no: string }>(
        `SELECT sa.member_id, sa.account_no FROM savings_account sa JOIN savings_product p ON p.id = sa.product_id
         WHERE p.category = 'WITHDRAWABLE DEPOSIT' AND sa.status IN ('ACTIVE','DORMANT')`)).map((r) => [r.member_id, r.account_no]));
      const rows = emps.map((e): ReportRow => ({ cells: { emp_no: e.employee_no, name: e.name,
        account: e.payment_mode === 'FOSA' && e.member_id ? `${fosa.get(e.member_id) ?? '—'} (FOSA)` : e.bank_account_no, mode: e.payment_mode, net: e.net } }));
    ...
    case 'master-roll': {
      ...
      return finish({ title: 'Master Roll', forLabel: 'Master Roll For', landscape: true,
        columns: [text('no', 'No.', { kind: 'int', width: '3%' }), text('emp_no', 'Emp No.'), text('name', 'Employee Name'), money('basic', 'Basic Salary'),
          ...
          money('gross', 'Gross Salary'), money('taxable', 'Taxable Pay'), money('statutory', 'Statutory Deduction'), money('sacco', 'Sacco Deduct.'), money('other', 'Other Deduct.'), money('net', 'Net Pay')],
        rows: numbered(rows) });
    }
    case 'pension': {
