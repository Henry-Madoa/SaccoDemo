page 52204074 "Treasury Requests"
{
    ...
    CardPageId = "Treasury Request";
    SourceTableView = where("Document Type" = const("Treasury Request"));
    Editable = false;
    ModifyAllowed = false;
    ...
    layout
    {
        ...
        area(FactBoxes)
        {
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"FOSA Transactions"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"FOSA Transactions"), "Document No." = FIELD("No.");
        ...
        area(Processing)
        {
            group("Request Approval")
            {
                ...
                action("Send Approval Request")
                {
                    ...
                    trigger OnAction();
                    begin
                        Rec.TestField("Destination No");
                        Rec.TestField(Amount);
                        ApprovalsMgmtExt.OnSendFOSATransactionForApproval(Rec);
                        CurrPage.Close();
                    end;
                }
                action("Cancel Approval Request")
                {
                    ...
                    trigger OnAction();
                    begin
                        ApprovalsMgmtExt.OnCancelFOSATransactionForApproval(Rec);
                        CurrPage.Close();
                    end;
                }
            }
    ...
    begin
        Rec."Document Type" := Rec."Document Type"::"Treasury Request";
    end;
