    ...
    layout
    {
        ...
        area(FactBoxes)
        {
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"Loan Batch Header"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"Loan Batch Header"), "Document No." = FIELD("No.");
    ...
    actions
    {
        area(Reporting)
        {
            action("Print Schedule")
            {
                ...
                begin
                    LoanBatch.Reset();
                    LoanBatch.SetRange("No.", Rec."No.");
                    if LoanBatch.FindSet() then Report.RunModal(Report::"Disbursement Schedule", true, false, LoanBatch);
                end;
            }
        }
        area(Processing)
        {
            ...
            action("Process Batch")
            {
                ...
                begin
                    ...
                    if Confirm('Do you want to Process the Batch?') then begin
                        LoansManagement.PostBatch(Rec."No.");
                    end;
                end;
            }
            action("Import Loans")
            {
                ...
                var
                    ...
                    LoansLookup: Page Loans;
                begin
                    LoanBatchLines.Reset();
                    LoanBatchLines.SetRange("No.", Rec."No.");
                    if LoanBatchLines.FindSet() then LoanBatchLines.DeleteAll(true);
                    Commit();
                    Loans.reset;
                    Loans.SetRange("Loan Balance", 0);
                    Loans.setrange("Loan Batch No.", '');
                    Loans.setrange(Status, Loans.Status::Approved);
                    Loans.SetRange(Posted, false);
                    Loans.SetFilter("Application Status", '%1|%2', Loans."Application Status"::Appraisal, Loans."Application Status"::Application);
                    if Loans.findset then begin
                        clear(LoansLookup);
                        LoansLookup.SetParameters(0, Rec."No.");
                        LoansLookup.LookUpMode := true;
                        LoansLookup.SetTableView(Loans);
