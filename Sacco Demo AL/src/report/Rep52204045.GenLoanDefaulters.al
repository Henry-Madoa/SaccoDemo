report 52204045 "Gen. Loan Defaulters"
{
    PreviewMode = Normal;
    UsageCategory = Administration;
    ApplicationArea = Basic, Suite;
    DefaultLayout = RDLC;
    RDLCLayout = './ssrs/GenerateLoanDefaulters.rdl';
    ...
    dataset
    {
        dataitem("Loan Application"; Loans)
        {
            ...
            column(Principal_Paid; PrincipalPaid)
            {
            }
            ...
            column(MonthlyPrincipal; MonthlyPrincipal)
            {
            }
            ...
            begin
                CompanyInformation.get;
                CompanyInformation.CalcFields(Picture);
                if "Loan Application"."Loan Amount" = 0 then CurrReport.Skip();
                ...
                PrincipalPaid := 0;
                LoanAge := 0;
                LoanAge := LoansMgt.GetLoanAge("No.", AsAtDate, DefPrincipal, PrincipalPaid, PrincipalDue, MonthlyPrincipal);
                if DefPrincipal = 0 then CurrReport.Skip();
                Deposits := 0;
                Deposits := LoansMgt.GetMemberDeposits("Member No.");
                AgeingGroup := '';
                if Members.Get("Loan Application"."Member No.") then begin
                    "Staff No" := Members."Payroll No.";
                    if "Staff No" = '' then "Staff No" := Members."Payroll No.";
                end;
