page 52203610 "Budget Holder Draft Budget Vie"
{
    // version BUDGETCaption = 'Draft Budget';
        ...
        area(processing)
        {
            group("F&unctions")
            {
                ...
                action("Copy Budget")
                {
                    ...
                    Image = CopyBudget;
                    Promoted = true;
                    //RunObject = Report "Copy Draft Budget";
    ...
    begin
        Draft:=true;
        if GLBudgetName.Status = GLBudgetName.Status::Approved then Draft:=false end;
    trigger OnInit()
    begin
        ...
        GlobalDim1Filter:=BudgetRoles."Global Dimension 1 Code";
        GlobalDim2Filter:=BudgetRoles."Global Dimension 2 Code";
        BudgetDim1Filter:=BudgetRoles."Global Dimension 3 Code";
        GLAccFilter:=BudgetRoles."Budget Acounts Range";
        GLSetup.Get;
        ColumnDimCode:=GLSetup."Global Dimension 1 Code";
        Draft:=true;
        if GLBudgetName.Status = GLBudgetName.Status::Approved then Draft:=false //
 end;
    trigger OnOpenPage()
    var
