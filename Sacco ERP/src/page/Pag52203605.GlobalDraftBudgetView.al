page 52203605 "Global Draft Budget View"
{
    // version BUDGETCaption = 'Budget';
    ...
    layout
    {
        area(content)
        {
            ...
            part(MatrixForm; "Draft Budget Matrix")
            {
                ApplicationArea = Suite;
            }
            group(Filters)
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            {
                ApplicationArea = Suite;
                Caption = 'Confirm Budget';
                Image = Confirm;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                ToolTip = 'This confirms your budget and takes it to the Consolidated Approved Budget';
                ...
                begin
                    ...
                    CurrPage.UPDATE;
                    UpdateMatrixSubform;*/
                    if Confirm('By confirming this budget, it will nolonger be available as draft. Do you wish to continue?', false) = true then begin
                        if GLBudgetName.Get(BudgetName)then begin
                            // GLBudgetName.Status := GLBudgetName.Status::Released;
                            // GLBudgetName.Modify;
                            Budgetmgmt.MoveDraftToMainBudget(GLBudgetName);
                        end;
                    end;
                end;
            }
    ...
    begin
        ...
        if ColumnDimCode = '' then case ColumnDimCode of Text001: // Period
 begin
    ...
    begin
        if DateFilter <> '' then begin
            ...
            if not PeriodFormMgt.FindDate('+', Calendar, PeriodType)then PeriodFormMgt.FindDate('+', Calendar, PeriodType::Day);
            Calendar.SetRange("Period Start");
        end;
        PeriodFormMgt.FindDate(SearchText, Calendar, PeriodType);
        GLAcc.SetRange("Date Filter", Calendar."Period Start", Calendar."Period End");
        if GLAcc.GetRangeMin("Date Filter") = GLAcc.GetRangeMax("Date Filter")then GLAcc.SetRange("Date Filter", GLAcc.GetRangeMin("Date Filter"));
        InternalDateFilter:=GLAcc.GetFilter("Date Filter");
        if(LineDimOption <> LineDimOption::Period) and (ColumnDimOption <> ColumnDimOption::Period)then DateFilter:=InternalDateFilter;
    end;
    ...
    begin
        if Confirm(Text003)then with GLBudgetEntry do begin
                SetRange("Budget Name", BudgetName);
                if BusUnitFilter <> '' then SetFilter("Business Unit Code", BusUnitFilter);
                if GLAccFilter <> '' then SetFilter("G/L Account No.", GLAccFilter);
                if DateFilter <> '' then SetFilter(Date, DateFilter);
                if GlobalDim1Filter <> '' then SetFilter("Global Dimension 1 Code", GlobalDim1Filter);
                if GlobalDim2Filter <> '' then SetFilter("Global Dimension 2 Code", GlobalDim2Filter);
                if BudgetDim1Filter <> '' then SetFilter("Budget Dimension 1 Code", BudgetDim1Filter);
                if BudgetDim2Filter <> '' then SetFilter("Budget Dimension 2 Code", BudgetDim2Filter);
                if BudgetDim3Filter <> '' then SetFilter("Budget Dimension 3 Code", BudgetDim3Filter);
                if BudgetDim4Filter <> '' then SetFilter("Budget Dimension 4 Code", BudgetDim4Filter);
                SetCurrentKey("Entry No.");
                if FindFirst then UpdateAnalysisView.SetLastBudgetEntryNo("Entry No." - 1);
