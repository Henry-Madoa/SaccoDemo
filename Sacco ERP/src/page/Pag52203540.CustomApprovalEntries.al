page 52203540 "Custom Approval Entries"
{
    ...
    begin
        Overdue:=Overdue::" ";
        if FormatField(Rec)then Overdue:=Overdue::Yes;
    ...
    local procedure FormatField(ApprovalEntry: Record "Approval Entry"): Boolean begin
        if Rec.Status in[Rec.Status::Created, Rec.Status::Open]then begin
            if ApprovalEntry."Due Date" < Today then exit(true);
            exit(false);
        end;
    end;
