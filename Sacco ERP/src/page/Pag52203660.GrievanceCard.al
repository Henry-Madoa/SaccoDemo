page 52203660 "Grievance Card"
{
    ...
    layout
    {
        area(content)
        {
            ...
            part(Control15; "Grievance subforms")
            {
                ApplicationArea = Basic, Suite;
        ...
        area(processing)
        {
            action("Submit Grievance")
            {
                ...
                begin
                    ...
                    if not Confirm('Are you sure you want to submit grievance no. ' + Format(Rec.No) + ' ?')then exit;
                    Disciplinarymanagement.ReportDisciplinaryCase(Rec);
                end;
            }
            action("Archive Grievance")
            {
                ApplicationArea = BasicHR;
                Image = Archive;
                Promoted = true;
                ...
                begin
                    ...
                    if not Confirm('Are you sure you want to Archive the grievance no. ' + Format(Rec.No) + ' ?')then exit;
    ...
    begin
        Rec.Validate(Type, Rec.Type::Grievance);
    end;
    trigger OnOpenPage()
    begin
        ControlAppearance;
