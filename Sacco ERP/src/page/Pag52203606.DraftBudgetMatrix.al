page 52203606 "Draft Budget Matrix"
{
    // version BUDGETCaption = 'Draft Budget Matrix';
    ...
    var Text001: Label 'Period';
    Text002: Label 'You may only edit column 1 to %1.';
    GLSetup: Record "General Ledger Setup";
    GLAccBudgetBuf: Record "Draft G/L Acc. Budget Buffer";
    ...
    local procedure FindRec(DimOption: Option "G/L Account", Period, "Business Unit", "Global Dimension 1", "Global Dimension 2", "Budget Dimension 1", "Budget Dimension 2", "Budget Dimension 3", "Budget Dimension 4"; var DimCodeBuf: Record "Dimension Code Buffer"; Which: Text[250]): Boolean var
        GLAcc: Record "G/L Account";
        BusUnit: Record "Business Unit";
        Period: Record Date;
        ...
        Found: Boolean;
    begin
        case DimOption of DimOption::"G/L Account": begin
            GLAcc."No.":=DimCodeBuf.Code;
            if GLAccFilter <> '' then GLAcc.SetFilter("No.", GLAccFilter);
            Found:=GLAcc.Find(Which);
            if Found then CopyGLAccToBuf(GLAcc, DimCodeBuf);
        end;
        DimOption::Period: begin
            ...
            if Found then CopyPeriodToBuf(Period, DimCodeBuf);
        end;
        DimOption::"Business Unit": begin
            BusUnit.Code:=DimCodeBuf.Code;
            if BusUnitFilter <> '' then BusUnit.SetFilter(Code, BusUnitFilter);
            Found:=BusUnit.Find(Which);
            if Found then CopyBusUnitToBuf(BusUnit, DimCodeBuf);
        end;
        DimOption::"Global Dimension 1": begin
            ...
            if Found then CopyDimValToBuf(DimVal, DimCodeBuf);
        end;
        DimOption::"Global Dimension 2": begin
        ...
        DimOption::"Budget Dimension 1": begin
            if BudgetDim1Filter <> '' then DimVal.SetFilter(Code, BudgetDim1Filter);
        ...
        DimOption::"Budget Dimension 2": begin
            if BudgetDim2Filter <> '' then DimVal.SetFilter(Code, BudgetDim2Filter);
        ...
        DimOption::"Budget Dimension 3": begin
            if BudgetDim3Filter <> '' then DimVal.SetFilter(Code, BudgetDim3Filter);
        ...
        DimOption::"Budget Dimension 4": begin
            if BudgetDim4Filter <> '' then DimVal.SetFilter(Code, BudgetDim4Filter);
    ...
    local procedure NextRec(DimOption: Option "G/L Account", Period, "Business Unit", "Global Dimension 1", "Global Dimension 2", "Budget Dimension 1", "Budget Dimension 2", "Budget Dimension 3", "Budget Dimension 4"; var DimCodeBuf: Record "Dimension Code Buffer"; Steps: Integer): Integer var
        GLAcc: Record "G/L Account";
        BusUnit: Record "Business Unit";
        Period: Record Date;
        DimVal: Record "Dimension Value";
    ...
    begin
        case DimOption of DimOption::"G/L Account": begin
            ...
            if ResultSteps <> 0 then CopyGLAccToBuf(GLAcc, DimCodeBuf);
        end;
        DimOption::Period: begin
            if DateFilter <> '' then Period.SetFilter("Period Start", DateFilter);
            Period."Period Start":=DimCodeBuf."Period Start";
            ResultSteps:=PeriodFormMgt.NextDate(Steps, Period, PeriodType);
            if ResultSteps <> 0 then CopyPeriodToBuf(Period, DimCodeBuf);
        end;
        DimOption::"Business Unit": begin
            BusUnit.Code:=DimCodeBuf.Code;
            if BusUnitFilter <> '' then BusUnit.SetFilter(Code, BusUnitFilter);
        ...
        DimOption::"Global Dimension 1": begin
            ...
            if ResultSteps <> 0 then CopyDimValToBuf(DimVal, DimCodeBuf);
        end;
        DimOption::"Global Dimension 2": begin
            if GlobalDim2Filter <> '' then DimVal.SetFilter(Code, GlobalDim2Filter);
            DimVal."Dimension Code":=GLSetup."Global Dimension 2 Code";
    ...
    begin
        with TheDimCodeBuf do begin
            ...
            if DateFilter <> '' then begin
                ...
                if Period2.GetRangeMax("Period End") < "Period End" then "Period End":=Period2.GetRangeMax("Period End");
            end;
    ...
    begin
        with TheDimCodeBuf do begin
            ...
            if TheBusUnit.Name <> '' then Name:=TheBusUnit.Name
            else
    ...
    begin
        with TheDimCodeBuf do begin
            ...
            "Show in Bold":=TheDimVal."Dimension Value Type" <> TheDimVal."Dimension Value Type"::Standard;
        end;
    end;
    ...
    begin
        ...
        DimOption::"Global Dimension 1", DimOption::"Global Dimension 2", DimOption::"Budget Dimension 1", DimOption::"Budget Dimension 2", DimOption::"Budget Dimension 3", DimOption::"Budget Dimension 4": begin
    ...
    local procedure SetCommonFilters(var TheGLAccBudgetBuf: Record "Draft G/L Acc. Budget Buffer")
    begin
        with TheGLAccBudgetBuf do begin
            Reset;
            SetRange("Budget Filter", GLBudgetName.Name);
            if BusUnitFilter <> '' then SetFilter("Business Unit Filter", BusUnitFilter);
            if GLAccFilter <> '' then SetFilter("G/L Account Filter", GLAccFilter);
            if DateFilter <> '' then SetFilter("Date Filter", DateFilter);
            if GlobalDim1Filter <> '' then SetFilter("Global Dimension 1 Filter", GlobalDim1Filter);
            if GlobalDim2Filter <> '' then SetFilter("Global Dimension 2 Filter", GlobalDim2Filter);
            if BudgetDim1Filter <> '' then SetFilter("Budget Dimension 1 Filter", BudgetDim1Filter);
            if BudgetDim2Filter <> '' then SetFilter("Budget Dimension 2 Filter", BudgetDim2Filter);
            if BudgetDim3Filter <> '' then SetFilter("Budget Dimension 3 Filter", BudgetDim3Filter);
            if BudgetDim4Filter <> '' then SetFilter("Budget Dimension 4 Filter", BudgetDim4Filter);
        end;
    end;
    local procedure SetDimFilters(var TheGLAccBudgetBuf: Record "Draft G/L Acc. Budget Buffer"; LineOrColumn: Option Line, Column)
    var
    ...
    begin
        ...
        with TheGLAccBudgetBuf do case DimOption of DimOption::"G/L Account": if DimCodeBuf.Totaling <> '' then GLAccBudgetBuf.SetFilter("G/L Account Filter", DimCodeBuf.Totaling)
                else
                    GLAccBudgetBuf.SetRange("G/L Account Filter", DimCodeBuf.Code);
            DimOption::Period: SetRange("Date Filter", DimCodeBuf."Period Start", DimCodeBuf."Period End");
            DimOption::"Business Unit": SetRange("Business Unit Filter", DimCodeBuf.Code);
            DimOption::"Global Dimension 1": if DimCodeBuf.Totaling <> '' then SetFilter("Global Dimension 1 Filter", DimCodeBuf.Totaling)
                else
                    SetRange("Global Dimension 1 Filter", DimCodeBuf.Code);
            DimOption::"Global Dimension 2": if DimCodeBuf.Totaling <> '' then SetFilter("Global Dimension 2 Filter", DimCodeBuf.Totaling)
                else
                    SetRange("Global Dimension 2 Filter", DimCodeBuf.Code);
            DimOption::"Budget Dimension 1": if DimCodeBuf.Totaling <> '' then SetFilter("Budget Dimension 1 Filter", DimCodeBuf.Totaling)
                else
                    SetRange("Budget Dimension 1 Filter", DimCodeBuf.Code);
            DimOption::"Budget Dimension 2": if DimCodeBuf.Totaling <> '' then SetFilter("Budget Dimension 2 Filter", DimCodeBuf.Totaling)
                else
                    SetRange("Budget Dimension 2 Filter", DimCodeBuf.Code);
            DimOption::"Budget Dimension 3": if DimCodeBuf.Totaling <> '' then SetFilter("Budget Dimension 3 Filter", DimCodeBuf.Totaling)
                else
                    SetRange("Budget Dimension 3 Filter", DimCodeBuf.Code);
            DimOption::"Budget Dimension 4": if DimCodeBuf.Totaling <> '' then SetFilter("Budget Dimension 4 Filter", DimCodeBuf.Totaling)
                else
    ...
    var
        GLBudgetEntry: Record "Draft Budget Entry";
    begin
        GLBudgetEntry.SetRange("Budget Name", GLBudgetName.Name);
        if GLAccBudgetBuf.GetFilter("G/L Account Filter") <> '' then GLAccBudgetBuf.CopyFilter("G/L Account Filter", GLBudgetEntry."G/L Account No.");
        if GLAccBudgetBuf.GetFilter("Business Unit Filter") <> '' then GLAccBudgetBuf.CopyFilter("Business Unit Filter", GLBudgetEntry."Business Unit Code");
        if GLAccBudgetBuf.GetFilter("Global Dimension 1 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Global Dimension 1 Filter", GLBudgetEntry."Global Dimension 1 Code");
        if GLAccBudgetBuf.GetFilter("Global Dimension 2 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Global Dimension 2 Filter", GLBudgetEntry."Global Dimension 2 Code");
        if GLAccBudgetBuf.GetFilter("Budget Dimension 1 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Budget Dimension 1 Filter", GLBudgetEntry."Budget Dimension 1 Code");
        if GLAccBudgetBuf.GetFilter("Budget Dimension 2 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Budget Dimension 2 Filter", GLBudgetEntry."Budget Dimension 2 Code");
        if GLAccBudgetBuf.GetFilter("Budget Dimension 3 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Budget Dimension 3 Filter", GLBudgetEntry."Budget Dimension 3 Code");
        if GLAccBudgetBuf.GetFilter("Budget Dimension 4 Filter") <> '' then GLAccBudgetBuf.CopyFilter("Budget Dimension 4 Filter", GLBudgetEntry."Budget Dimension 4 Code");
        if GLAccBudgetBuf.GetFilter("Date Filter") <> '' then GLAccBudgetBuf.CopyFilter("Date Filter", GLBudgetEntry.Date)
        else
            GLBudgetEntry.SetRange(Date, 0D, DMY2Date(31, 12, 9999));
        with GLBudgetEntry do if(GetFilter("Global Dimension 1 Code") <> '') or (GetFilter("Global Dimension 2 Code") <> '') or (GetFilter("Business Unit Code") <> '')then SetCurrentKey("Budget Name", "G/L Account No.", "Business Unit Code", "Global Dimension 1 Code")
            else
                SetCurrentKey("Budget Name", "G/L Account No.", Date);
        PAGE.Run(0, GLBudgetEntry);
    end;
    local procedure CalcAmount(SetColumnFilter: Boolean): Decimal begin
        SetCommonFilters(GLAccBudgetBuf);
        SetDimFilters(GLAccBudgetBuf, 0);
    ...
    var
        GLAcc: Record "G/L Account";
    begin
        if DimCodeToOption(LineDimCode) = 0 then GLAcc.Get(Rec.Code)
        else
        begin
            if GLAccFilter <> '' then GLAcc.SetFilter("No.", GLAccFilter);
            GLAcc.FindFirst;
        ...
        with GLAcc do begin
            SetRange("Budget Filter", GLBudgetName.Name);
            if BusUnitFilter <> '' then SetFilter("Business Unit Filter", BusUnitFilter);
            if GLAccFilter <> '' then SetFilter("No.", GLAccFilter);
            if GlobalDim1Filter <> '' then SetFilter("Global Dimension 1 Filter", GlobalDim1Filter);
            if GlobalDim2Filter <> '' then SetFilter("Global Dimension 2 Filter", GlobalDim2Filter);
        end;
    ...
    local procedure FormatStr(): Text begin
