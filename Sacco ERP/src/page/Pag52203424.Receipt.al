page 52203424 "Receipt"
{
    ...
    layout
    {
        area(Content)
        {
            group(General)
            {
                ...
                field("Manual Receipt No."; Rec."Manual Receipt No.")
                {
                    ApplicationArea = Basic, Suite;
                }
            ...
            part("ReceiptDetails"; "Receipt Lines")
            {
        ...
        area(Processing)
        {
            group("Request Approval")
            {
                ...
                action("Send Approval Request")
                {
                    ...
                    Visible = ((Rec.Status = Rec.Status::Open) and not PostingVisibility);
                    ...
                    begin
                        ...
                        ApprovalsMgmtExt.OnSendReceiptHeaderForApproval(Rec) // else begin
            ...
            action(Post)
            {
                ...
                begin
                    if Confirm('Do you want to Post?')then begin
                        If Rec.Amount >= Rec."Approval Limit" then begin
                            if Confirm('Do you want to send for Approval?')then ApprovalsMgmtExt.OnSendReceiptHeaderForApproval(Rec)end
                        else
                            ReceiptManagement.PostReceipt(Rec);
                        //Commit();
                        // Rec.Reset();
            ...
            action("Import from Excel")
            {
                ...
                begin
                    IF not Confirm('Are you sure on Uploading Excel into the Receipt?', true)then exit;
                end;
            }
        }
    }
    ...
    begin
        ...
        if Rec.Amount >= Rec."Approval Limit" then PostingVisibility:=false
        else
