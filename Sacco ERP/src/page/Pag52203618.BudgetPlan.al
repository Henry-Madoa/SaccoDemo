page 52203618 "Budget Plan"
{
    ...
    layout
    {
        area(Content)
        {
            ...
            part(BudgetPlanLines; "Budget Plan Lines")
            {
                ApplicationArea = Basic, Suite;
                Editable = Rec.Status = Rec.Status::Open;
        ...
        area(factboxes)
        {
            ...
            part(Control3; "Budget Plan Factbox")
            {
                ApplicationArea = Suite;
                Provider = BudgetPlanLines;
    ...
    actions
    {
        area(Processing)
        {
            action("Schedule Budget Plan")
            {
                ...
                PromotedIsBig = true;
                Visible = ((Rec.Status = Rec.Status::Approved) and (Rec.Scheduled = false));

                trigger OnAction()
                begin
                    BudgetMgmt.GenerateBudgetPlanSchedule(Rec);
                end;
            }
            action(Post)
            {
                ...
                PromotedCategory = Process;
                PromotedIsBig = true;
                Visible = ((Rec.Status = Rec.Status::Approved) and (Rec.Scheduled = true) and (Rec.Posted = false));

                trigger OnAction()
                begin
                    BudgetMgmt.PostBudgetPlan(Rec);
                end;
            }
            action(DocAttach)
            {
