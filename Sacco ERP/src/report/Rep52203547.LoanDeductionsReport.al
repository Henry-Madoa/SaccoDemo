report 52203547 "Loan Deductions Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Loan Deductions Report.rdlc';

    dataset
    {
        dataitem("Payroll Period Transaction"; "Payroll Period Transaction")
        {
            DataItemTableView = WHERE("Coop Parameters"=FILTER(loan));
            ...
            begin
                ...
                PayrollTransactionCode.SetRange("Principal Loan", "Payroll Period Transaction"."Transaction Code");
                if PayrollTransactionCode.FindFirst then begin
                    PayrollPeriodTransaction.Reset;
                    PayrollPeriodTransaction.SetRange("Employee Code", "Payroll Period Transaction"."Employee Code");
                    PayrollPeriodTransaction.SetRange("Payroll Period", "Payroll Period Transaction"."Payroll Period");
                    PayrollPeriodTransaction.SetRange("Transaction Code", PayrollTransactionCode.Code);
                    if PayrollPeriodTransaction.FindFirst then InterestAmount:=PayrollPeriodTransaction.Amount;
                end;
            end;
