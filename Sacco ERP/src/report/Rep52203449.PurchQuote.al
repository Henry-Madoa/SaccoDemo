report 52203449 "Purch Quote"
{
    // version NAVW113.00DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Purch Quote.rdlc';
    Caption = 'Sales - Quote';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(WorkDesc; "Sales Header"."Work Description")
            {
            }
            ...
            column(ProjectCaption; ProjectLbl)
            {
            }
            column(QuoteCaption; QuoteLbl)
            {
            }
            column(PhysicalCaption; PhysicalLbl)
            {
            }
            column(CourierCaption1; CourierLbl1)
            {
            }
            ...
            begin
                begin
                    //CurrReport.Language := Language.GetLanguageID("Language Code");            FormatAddressFields("Sales Header");
                    if Print then begin
                        if CurrReport.UseRequestPage and ArchiveDocument or not CurrReport.UseRequestPage and (SalesSetup."Archive Quotes" = SalesSetup."Archive Quotes"::Always)then ArchiveManagement.StoreSalesDocument("Sales Header", LogInteraction);
            ...
            begin
                ...
                if Find('-') and Task.WritePermission then if Print and (NoOfRecords = 1)then if Confirm(Text007)then CreateTask;
            end;
            trigger OnPreDataItem()
            begin
                NoOfRecords:=Count;
    ...
    requestpage
    {
        ...
        begin
            ...
            case SalesSetup."Archive Quotes" of SalesSetup."Archive Quotes"::Never: ArchiveDocument:=false;
            SalesSetup."Archive Quotes"::Always: ArchiveDocument:=true;
            end;
        end;
        trigger OnOpenPage()
        begin
    ...
    var Text004: Label 'Sales - Quote %1', Comment = '%1 = Document No.';
