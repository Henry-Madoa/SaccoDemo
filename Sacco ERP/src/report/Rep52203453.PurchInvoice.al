report 52203453 "Purch - Invoice"
{
    // version NAVW113.00DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Purch - Invoice.rdlc';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(VATCounterLCY; "Integer")
                    {
                        ...
                        begin
                            VATAmountLine.GetLine(Number);
                            VALVATBaseLCY:=VATAmountLine.GetBaseLCY("Purch. Inv. Header"."Posting Date", "Purch. Inv. Header"."Currency Code", "Purch. Inv. Header"."Currency Factor");
                            VALVATAmountLCY:=VATAmountLine.GetAmountLCY("Purch. Inv. Header"."Posting Date", "Purch. Inv. Header"."Currency Code", "Purch. Inv. Header"."Currency Factor");
                        end;
                        trigger OnPreDataItem()
                        begin
                            if(not GLSetup."Print VAT specification in LCY") or ("Purch. Inv. Header"."Currency Code" = '')then CurrReport.Break;
                            SetRange(Number, 1, VATAmountLine.Count);
                ...
                begin
                    if not IsReportInPreviewMode then CODEUNIT.Run(CODEUNIT::"Purch. Inv.-Printed", "Purch. Inv. Header");
                end;
                trigger OnPreDataItem()
                begin
                    OutputNo:=1;
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode then if "Purch. Inv. Header".FindSet then repeat SegManagement.LogDocument(14, "Purch. Inv. Header"."No.", 0, 0, DATABASE::Vendor, "Purch. Inv. Header"."Buy-from Vendor No.", "Purch. Inv. Header"."Purchaser Code", '', "Purch. Inv. Header"."Posting Description", '');
    ...
    var Text004: Label 'Purchase - Invoice %1', Comment = '%1 = Document No.';
    ...
    "Payment:": Label 'Payment Terms:-     50% With LPO';
    local procedure DocumentCaption(): Text[250]begin
        if "Purch. Inv. Header"."Prepayment Invoice" then exit(Text010);
        exit(Text004);
    end;
