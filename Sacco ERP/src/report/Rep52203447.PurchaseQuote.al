report 52203447 "Purchase Quote"
{
    // version NAVW113.00DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Purchase Quote.rdlc';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(Amount; PurchLine.Amount)
            {
            }
            column(TotalVAT; VAT)
            {
            }
            column(AmtVAT; PurchLine."Amount Including VAT")
            {
            }
            column(UnitCost; PurchLine."Unit Cost (LCY)")
            {
            }
            column(VAT; PurchLine."VAT %")
            {
            }
            column(AmtTotal; "Purchase Header".Amount)
            {
            }
            column(AmtInclVAT; "Purchase Header"."Amount Including VAT")
            {
            }
            column(UOM; PurchLine."Unit of Measure")
            {
            }
            ...
            begin
                ...
                VAT:="Purchase Header"."Amount Including VAT" - "Purchase Header".Amount;
            end;
        }
    }
    requestpage
    {
        ...
        begin
            ...
            case PurchSetup."Archive Quotes" of PurchSetup."Archive Quotes"::Never: ArchiveDocument:=false;
            PurchSetup."Archive Quotes"::Always: ArchiveDocument:=true;
            end;
        end;
        trigger OnOpenPage()
        begin
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode then if "Purchase Header".FindSet then repeat "Purchase Header".CalcFields("No. of Archived Versions");
                    SegManagement.LogDocument(11, "Purchase Header"."No.", "Purchase Header"."Doc. No. Occurrence", "Purchase Header"."No. of Archived Versions", DATABASE::Vendor, "Purchase Header"."Pay-to Vendor No.", "Purchase Header"."Purchaser Code", '', "Purchase Header"."Posting Description", '');
    ...
    var Text002: Label 'Purchase - Quote %1', Comment = '%1 = Document No.';
