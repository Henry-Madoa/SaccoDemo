report 52203524 "Payroll Allowances Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Payroll Allowances Report.rdlc';

    dataset
    {
        dataitem("Payroll Period Transaction"; "Payroll Period Transaction")
        {
            DataItemTableView = SORTING("Group Order", "Transaction Code", "Period Month", "Period Year", Membership, "Reference No", "Department Code")WHERE("Group Text"=FILTER('ALLOWANCE'), "Transaction Type"=CONST(Income));
            ...
            begin
                ...
                if DimensionValue.Get(2, "Payroll Period Transaction"."Global Dimension 2 Code")then DepartmentName:=DimensionValue.Name;
            end;
        }
    }
    requestpage
