report 52203452 "Sale - Invoice"
{
    // version NAVW113.00
    DefaultLayout = RDLC;
    RDLCLayout = './ssrs/Sale - Invoice.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(PhysicalCaption; PhysicalLbl)
            {
            }
            column(CourierCaption; CourierLbl)
            {
            }
            column(Courier1Caption; CourierLbl1)
            {
            }
            column(HandDevCaption; HandDevLbl)
            {
            }
            column(OfferCaption; OfferLbl)
            {
            }
            column(AccountCaption; AccountLbl)
            {
            }
            column(PINCaption; PINLbl)
            {
            }
            column(PaymentCaption; "Payment:")
            {
            }
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(HdrOrderNo_SalesInvHdr; "Sales Header"."No.")
                    {
                    }
            ...
            begin
                ...
                OnGetReferenceText("Sales Header", ReferenceText, Handled);
            end;
        }
    }
    requestpage
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode then if "Sales Header".FindSet then repeat // if "Sales Header"."Bill-to Contact No." <> '' then
 //     SegManagement.LogDocument(SegManagement.SalesInvoiceInterDocType, "Sales Header"."No.", 0, 0, DATABASE::Contact, "Sales Header"."Bill-to Contact No.", "Sales Header"."Salesperson Code", "Sales Header"."Campaign No.", "Sales Header"."Posting Description", '')
    ...
    PhysicalLbl: Label 'Physical address:               Moi South Lake Road.';
    CourierLbl: Label 'Courier:                             Specialised Cargo Area. 1st Avenue';
    CourierLbl1: Label '                                         Jomo Kenyatta International Airport, Nairobi, Kenya';
    HandDevLbl: Label 'Hand Delivery:                  "Dropping Zone" No 15, Revlon Proffessional Plaza, Tuban Road, Nairobi';
    OfferLbl: Label 'Offering Top Quality Engineering Services';
    AccountLbl: Label 'ACCOUNTS ARE DUE ON DEMAND';
    PINLbl: Label 'PIN:';
    "Payment:": Label 'Payment Terms  :-    30 Days';
    ...
    begin
        ...
        if "Sales Header"."No." = '' then case "Sales Invoice Line".Type of "Sales Invoice Line".Type::Item: GenerateBufferFromValueEntry("Sales Invoice Line");
    ...
    begin
        ...
        SalesShipmentLine.SetCurrentKey("No.", "Order Line No.");
        SalesShipmentLine.SetRange("No.", "Sales Header"."No.");
        ...
        if SalesShipmentLine.Find('-')then repeat if "Sales Header"."Get Shipment Used" then CorrectShipment(SalesShipmentLine);
    ...
    local procedure DocumentCaption(): Text[250]var
        DocCaption: Text;
    begin
    ...
    local procedure GetUOMText(UOMCode: Code[10]): Text[10]var
    ...
    begin
        ...
        if not Customer.Get(CustLedgerEntry."Customer No.")then LineFeeNoteOnReportHist.SetRange("Cust. Ledger Entry No", CustLedgerEntry."Entry No.");
    ...
    local procedure CalcVATAmountLineLCY(SalesInvoiceHeader: Record "Sales Header"; TempVATAmountLine2: Record "VAT Amount Line" temporary; var TempVATAmountLineLCY2: Record "VAT Amount Line" temporary; var VATBaseRemainderAfterRoundingLCY2: Decimal; var AmtInclVATRemainderAfterRoundingLCY2: Decimal)
    var
        VATBaseLCY: Decimal;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetDocumentCaption(SalesInvoiceHeader: Record "Sales Header"; var DocCaption: Text)
    begin
    end;
    [IntegrationEvent(false, false)]
    procedure OnGetReferenceText(SalesInvoiceHeader: Record "Sales Header"; var ReferenceText: Text[80]; var Handled: Boolean)
    begin
    end;
}
