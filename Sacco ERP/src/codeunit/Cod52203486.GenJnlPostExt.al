codeunit 52203486 "Gen. Jnl.-Post Ext"
{
    ...
    var JournalsScheduledMsg: Label 'Journal lines have been scheduled for posting.';
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if GenJnlTemplate.Recurring and (GetFilter("Posting Date") <> '')then FieldError("Posting Date", Text000);
            ...
            if "Account Type" = "Account Type"::"Fixed Asset" then begin
                ...
                if FALedgEntry.FindFirst and "Depr. Acquisition Cost" and not HideDialog then if not ConfirmManagement.GetResponseOrDefault(StrSubstNo(Text005, FieldCaption("Depr. Acquisition Cost")), true)then exit;
            end;
            if not HideDialog then if not GenJnlPostBatch.ConfirmPostingUnvoidableChecks("Journal Batch Name", "Journal Template Name")then exit;
            ...
            if GeneralLedgerSetup."Post with Job Queue" and not PreviewMode then begin
                ...
                if GenJnlsScheduled then Message(JournalsScheduledMsg);
            end
            else
            begin
                IsHandled:=false;
                ...
                if "Line No." = 0 then Message(Text002)
                else if TempJnlBatchName = "Journal Batch Name" then Message(Text003)
                    else
