table 52203477 "Accrual Period"
{
    Caption = 'Accounting Period';
    LookupPageID = "Accrual Periods";
    ...
    fields
    {
        ...
        field(3; "New Fiscal Year"; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                Rec.Testfield("Date Locked", false);
                if "New Fiscal Year" then begin
                    if not InvtSetup.Get then exit;
        ...
        field(5806; "Calender Code"; Code[20])
        {
    ...
    keys
    {
        key(Key1; "Starting Date", "Calender Code")
        {
        }
    ...
    begin
        ...
        if AccountingPeriod2.Find('>')then AccountingPeriod2.TestField("Date Locked", false);
    end;
    trigger OnRename()
    begin
    //Rec.Testfield("Date Locked",FALSE);
    ...
    var Text000: Label '<Month Text>';
    AccountingPeriod2: Record "Accrual Period";
    ...
    [Scope('Cloud')]
    procedure GetFiscalYearEndDate(ReferenceDate: Date): Date var
        AccountingPeriod: Record "Accounting Period";
    begin
        if AccountingPeriod.IsEmpty then exit(CalcDate('<CY>', ReferenceDate));
        with AccountingPeriod do begin
            SetRange("New Fiscal Year", true);
            SetRange("Starting Date", 0D, ReferenceDate);
            if FindLast then SetRange("Starting Date");
            if Find('>')then exit("Starting Date" - 1);
        end;
    end;
    [Scope('Cloud')]
    procedure GetFiscalYearStartDate(ReferenceDate: Date): Date var
        AccountingPeriod: Record "Accounting Period";
    begin
        if AccountingPeriod.IsEmpty then exit(CalcDate('<-CY>', ReferenceDate));
        with AccountingPeriod do begin
            SetRange("New Fiscal Year", true);
            SetRange("Starting Date", 0D, ReferenceDate);
            if FindLast then exit("Starting Date")end;
    end;
}
