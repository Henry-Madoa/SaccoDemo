page 52204142 "Money Laundary Checks"
{
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                field("Uncleared Balance"; Rec."Uncleared Balance")
                {
                    ApplicationArea = Basic, Suite;
                }
        ...
        area(Factboxes)
        {
            ...
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"Money Laundary Check"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"Money Laundary Check"), "Document No." = FIELD("No.");
    ...
    actions
    {
        area(Processing)
        {
            group("Request Approval")
            {
                ...
                action("Send Approval Request")
                {
                    ...
                    begin
                        ...
                        ApprovalsMgmtExt.OnSendMoneyLaundaryCheckForApproval(Rec);
                        CurrPage.Close();
                    end;
                }
                action("Cancel Approval Request")
                {
                    ...
                    trigger OnAction();
                    begin
                        ApprovalsMgmtExt.OnCancelMoneyLaundaryCheckForApproval(Rec);
                        CurrPage.Close();
                    end;
                }
            }
            ...
            action(Clear)
            {
                ...
                Visible = ((Rec.Status = Rec.Status::Approved) and not Rec.Cleared);

                trigger OnAction()
                begin
                    MoneyLaundaryMgmt.PostMoneyLaundary(Rec);
                end;
            }
        }
    }
    ...
    var
        ...
        MoneyLaundaryMgmt: Codeunit "Money Laundary Mgmt.";
}
