page 52204097 "ATM Application"
{
    ...
    layout
    {
        area(Content)
        {
            ...
            group("Card Details")
            {
                Editable = ((Rec.Status = Rec.Status::Approved) and (not Rec.Processed));
            ...
            group("Collection Details")
            {
                ...
                Editable = (Rec.Processed and (not Rec.Collected));
        ...
        area(FactBoxes)
        {
            ...
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"ATM Application"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"ATM Application"), "Document No." = FIELD("No.");
    ...
    actions
    {
        ...
        area(Processing)
        {
            ...
            action("Post Linking")
            {
                ...
                PromotedIsBig = true;
                Visible = ((Rec.Status = Rec.Status::Approved) and ((Rec."Application Type" = Rec."Application Type"::New) or (Rec."Application Type" = Rec."Application Type"::Reactivation)) and (not Rec.Processed));
                ...
                begin
                    ...
                    if Confirm('Do you want to Post') then
                        MemberMgt.PostATMLinking(Rec."No.");
                end;
            }
            action("Post Collection")
            {
                ...
                PromotedCategory = Process;
                PromotedIsBig = true;
                Visible = ((Rec.Processed) and (not Rec.Collected));
                trigger OnAction()
                var
                    MemberMgt: Codeunit "Member Management";
                begin
                    if Confirm('Do you want to Post Collection') then
                        MemberMgt.PostATMCollection(Rec."No.");
                end;
            }
            action("Post DeLinking")
            {
                ApplicationArea = Basic, Suite;
                Image = Process;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                Visible = ((Rec.Status = Rec.Status::Approved) and (Rec."Application Type" = Rec."Application Type"::Delinking) and (not Rec.Processed));
                ...
                begin
                    Rec.TestField(Processed, false);
                    if Confirm('Do you want to Post') then MemberMgt.PostATMDeLinking(Rec."No.");
                end;
            }
        }
    }
    ...
    begin
        ...
        if Rec.Processed then
            CardNo := MemberMgt.MaskCardNo(Rec."Card No.")
        else
            CardNo := Rec."Card No.";
    end;
