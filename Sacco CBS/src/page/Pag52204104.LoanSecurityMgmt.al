page 52204104 "Loan Security Mgmt."
{
    ...
    layout
    {
        area(Content)
        {
            ...
            part("Substitution Lines"; "Loan Security Mgmt Lines")
            {
                Editable = Rec.Status = Rec.Status::Open;
                ApplicationArea = Basic, Suite;
        ...
        area(FactBoxes)
        {
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"Loan Security Mgmt"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"Loan Security Mgmt"), "Document No." = FIELD("No.");
        ...
        area(Processing)
        {
            ...
            action("Populate Guarantor Lines")
            {
                ...
                begin
                    if Confirm('Do you want to populate guaranteed loans?') then begin
                        LoansMgt.PopulateGuarantorSubLines(Rec."No.");
                    end;
                end;
            }
            action(Process)
            {
                ...
                var
                    LoansMgt: Codeunit "Loans Management";
                begin
                    IF NOT CONFIRM('Are you sure you want to Process the document?') THEN EXIT;
                    LoansMgt.ProcessGuarantorSubstitution(Rec."No.");
                end;
            }
        }
    }
    ...
    begin
        if GuiAllowed then begin
            Rec.Source := Rec.Source::"Core Banking";
