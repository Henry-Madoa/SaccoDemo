page 52204242 "Product Editings"
{
    ...
    CardPageId = "Product Editing";
    Editable = false;
    ModifyAllowed = false;
    SourceTable = "Products Management";
    SourceTableView = where("Document Type" = const(Update));
    ...
    actions
    {
        area(Processing)
        {
            action(Process)
            {
                ...
                trigger OnAction()
                begin
                    ProductManagement.ProcessProductEditing(Rec);
                    CurrPage.Close;
                end;
            }
            action(Charges)
    ...
    begin
        Rec."Document Type" := Rec."Document Type"::Update;
    end;
