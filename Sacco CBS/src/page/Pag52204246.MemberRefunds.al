page 52204246 "Member Refunds"
{
    ...
    ModifyAllowed = false;
    Editable = false;
    SourceTable = "Member Withdrawal";
    SourceTableView = where("Document Type" = const(Refund));
    ...
    layout
    {
        ...
        area(FactBoxes)
        {
            ...
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(Database::"Member Withdrawal"), "No." = FIELD("No.");
            }
            part(Control27; "Pending Approval FactBox")
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID" = CONST(Database::"Member Withdrawal"), "Document No." = FIELD("No.");
    ...
    actions
    {
        ...
        area(Reporting)
        {
            action(Print)
            {
                ...
                begin
                    MemberExit.Reset();
                    MemberExit.SetRange("No.", Rec."No.");
                    if MemberExit.FindSet() then Report.Run(Report::"Member Withdrawal", true, false, MemberExit);
                end;
            }
        }
        area(Processing)
        {
            ...
            action("Validate Assets & Liabilities")
            {
                ...
                begin
                    if Confirm('Do you want to validate?') then begin
                        MemberMgt.PopulateMemberAssetsLiabilities(Rec."No.");
                    end;
                end;
            }
            action("Bulk Post")
            {
                ...
                trigger OnAction()
                var
                    MemberRefund: Record "Member Withdrawal";
                begin
                    if Confirm('Do you want to Post Bulk Refund?') then begin
                        MemberRefund.RESET;
                        CurrPage.SETSELECTIONFILTER(MemberRefund);
                        if MemberRefund.FindSet then begin
                            repeat
                                MemberMgt.PostMemberWithdrawal(MemberRefund."No.");
                            until MemberRefund.NEXT = 0;
                        end;
                    end;
                end;
            }
            action(Post)
            {
                ...
                begin
                    if Confirm('Do you want to Post?') then begin
                        MemberMgt.PostMemberWithdrawal(Rec."No.");
                        CurrPage.Close();
                    end;
                end;
            }
            action("Re-Open")
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Do you want to Reopen the Withdrawal Request?') then begin
