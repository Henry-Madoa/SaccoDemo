codeunit 52204017 "Channels Integrations"
{
    ...
    var
        ...
        ChannelGuarantorRequests: Record "Channel Guarantor Requests";
        ...
        AccountsArray, TransactionsArray, LoansArray, KinArray, NomineesArray, SubscriptionsArray, GuaranteesArray, GuarantorsArray, ChannelGuaranteesArray : JsonArray;
        LineJson: JsonObject;
        ResultText: Text;
        BookValue, VendorBalance, AvailableBalance, MinimumBalance : Decimal;
    ...
    begin
        ...
        AvailableBalance := BookValue - Member."Uncleared Funds" - MinimumBalance - GetPendingChannelsTransactions(Member."No.");
        ...
        // ---- Guarantees Requests (loans this member guaranteed for others) ----
        ChannelGuarantorRequests.Reset();
        ChannelGuarantorRequests.SetRange("Member No", Member."No.");
        ChannelGuarantorRequests.SetRange("Loan Submitted", false);
        ChannelGuarantorRequests.SetRange(Status, ChannelGuarantorRequests.Status::Open);
        if ChannelGuarantorRequests.FindSet() then
            repeat
                if ChannelLoanApplication.Get(ChannelGuarantorRequests."Loan No") then begin
                    ChannelLoanApplication.CalcFields("Loan Balance");
                    if ChannelLoanApplication."Portal Status" = ChannelLoanApplication."Portal Status"::New then begin
                        Clear(LineJson);
                        LineJson.Add('MemberNo', ChannelLoanApplication."Member No.");
                        LineJson.Add('MemberName', ChannelLoanApplication."Member Name");
                        LineJson.Add('LoanProduct', ChannelLoanApplication."Product Description");
                        LineJson.Add('LoanNo', ChannelLoanApplication."No.");
                        LineJson.Add('Amount', Format(ChannelLoanApplication."Applied Amount"));
                        LineJson.Add('RequestedAmount', Format(ChannelGuarantorRequests."Requested Amount"));
                        ChannelGuaranteesArray.Add(LineJson);
                    end;
                end;
            until LoanGuarantees.Next() = 0;
        RootJson.Add('GuarantorshipRequest', ChannelGuaranteesArray);
    ...
    begin
        ...
        if Member.GET(MemberNo) THEN BEGIN
            ...
            AvailableBalance := BookValue - Member."Uncleared Funds" - MinimumBalance - GetPendingChannelsTransactions(MemberNo);
    ...
    begin
        ...
        ChannelGuarantorRequests.Reset();
        ChannelGuarantorRequests.SetRange("Member No", MemberNo);
        ChannelGuarantorRequests.SetRange("Loan No", LoanNo);
        if RequestType = RequestType::Guarantor then
            ChannelGuarantorRequests.SetRange("Request Type", ChannelGuarantorRequests."Request Type"::Guarantor)
        else
            ChannelGuarantorRequests.SetRange("Request Type", ChannelGuarantorRequests."Request Type"::Witness);
        if ChannelGuarantorRequests.FindSet() then begin
            if ResponseType = ResponseType::Accepted then begin
                ChannelGuarantorRequests.Status := ChannelGuarantorRequests.Status::Approved;
                if RequestType = RequestType::Witness then begin
                    ...
                    ChannelGuarantorRequests."Amount Accepted" := Amount;
                    LoanGuarantees.Reset();
                ...
                ChannelGuarantorRequests.Status := ChannelGuarantorRequests.Status::Rejected;
            ChannelGuarantorRequests."Amount Accepted" := Amount;
            ChannelGuarantorRequests."Responded On" := CurrentDateTime;
            ChannelGuarantorRequests.Modify();
            responseCode := '00';
            if Member.Get(ChannelGuarantorRequests.Applicant) then begin
                if ChannelGuarantorRequests."Request Type" = ChannelGuarantorRequests."Request Type"::Guarantor then
                    SMSText := 'Dear ' + Member."Full Name" + ', ' + ChannelGuarantorRequests."Member Name" + ' has accepted your guarantee request'
                else
                    SMSText := 'Dear ' + Member."Full Name" + ', ' + ChannelGuarantorRequests."Member Name" + ' has accepted your loan witness request';
    ...
    var
        MemberNo: Code[20];
        TempResponse: BigText;
        BookValue, AvailableBalance, MinimumBalance : Decimal;
        LoansManagement: Codeunit "Loans Management";
        SaccoProduct: Record "Sacco Products";
    begin
        Clear(TempResponse);
    ...
    begin
        ...
        PesaLinkTransactions."Origanal Msg ID" := originalMsgId;
        PesaLinkTransactions."Transaction Direction" := direction;
        PesaLinkTransactions."Transaction Time" := transactionTime;
        PesaLinkTransactions."Transaction DateTime" := transactionDateTime;
