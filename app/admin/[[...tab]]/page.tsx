...
import { requireUser, currentCanAction } from '@/lib/session';
// The four shared Finance master tables the Setup Pool now owns — see the Finance tabs below.
...
import {
  PaymentTermsFormButton, PaymentMethodFormButton, CustomerPostingGroupFormButton,
  DeleteCustomerPostingGroupButton, DeletePaymentTermsButton, DeletePaymentMethodButton,
} from '@/app/receivables/receivables-forms';
...
import {
  listJobGrades, listContractTypes, listTerminationReasons, listClearanceSections,
} from '@/lib/hrSetup';
import {
  listLeaveTypes, listLeaveCalendars, getCurrentLeaveCalendar, listHolidays, listAccrueMatrix,
} from '@/lib/leaveManagement';
import {
  getPayrollSetup, listPostingGroups, listPayeBands, listNssfTiers, listTransactionCodes,
...
async function ContractTypesTab() {
...
async function TerminationReasonsTab() {
...
async function ClearanceSectionsTab() {
...
async function LeaveCalendarTab() {
...
async function AccrueMatrixTab() {
...
async function PayrollSetupTab() {
...
async function TransactionCodesTab() {
...
/* ------------------------------------------------------- Finance: shared master data
 *
 * Payment Terms, Payment Methods and the two Posting Groups are admin master data that the
 * Customer and Vendor cards, the Receipt and the Payment Voucher all relate to — so they are
 * defined once here rather than inside whichever module happened to need them first. The forms
 * themselves are the Receivables / Payables ones, reused rather than reimplemented.
 */
...
async function CustomerPostingGroupsTab() {
...
async function VendorPostingGroupsTab() {
  const [rows, canManage, accounts] = await Promise.all([
