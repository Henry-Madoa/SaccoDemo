...
import { addApplicationAttachment, removeApplicationAttachment } from '@/app/actions/applicationAttachments';
import { ATTACHMENT_CATEGORIES } from '@/lib/constants';
import { formatBytes, formatDateTime } from '@/lib/format';
import type { MemberApplicationAttachment, UploadedFile } from '@/lib/types';

export interface ApplicationAttachmentPanelProps {
...
export function ApplicationAttachmentPanel({
  applicationNo, attachments, canManage, mediaEnabled, onChanged,
}: ApplicationAttachmentPanelProps) {
  ...
  const attach = async (file: UploadedFile) => {
    setBusy(true);
    try {
      const res = await addApplicationAttachment(applicationNo, file, category);
  ...
  const remove = async (a: MemberApplicationAttachment) => {
    setBusy(true);
    try {
      const res = await removeApplicationAttachment(applicationNo, a.id);
