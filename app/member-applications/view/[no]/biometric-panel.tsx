...
import { saveMemberApplicationBiometric, type BiometricKind } from '@/app/actions/media';
...
export interface ApplicationBiometricPanelProps {
  applicationNo: string;
  images: Record<BiometricKind, string | null>;
  canEdit: boolean;
  mediaEnabled: boolean;
  /** Called after a successful upload/removal, for callers holding their own copy of the images. */
  onChanged?: () => void;
}

/** Front/back ID scan, signature and fingerprint capture — tabbed, one slot each, on the application card. */
export function ApplicationBiometricPanel({
  applicationNo, images, canEdit, mediaEnabled, onChanged,
}: ApplicationBiometricPanelProps) {
...
function BiometricSlot({ applicationNo, kind, label, src, canEdit, mediaEnabled, onChanged }: {
  applicationNo: string; kind: BiometricKind; label: string; src: string | null;
  canEdit: boolean; mediaEnabled: boolean; onChanged?: () => void;
}) {
  ...
  const apply = async (file: UploadedFile | null) => {
    setBusy(true);
    try {
      const res = await saveMemberApplicationBiometric(applicationNo, kind, file);
