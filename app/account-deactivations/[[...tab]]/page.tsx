...
import {
  NewAccountDeactivationButton, SubmitButton, CancelApprovalButton, ProcessButton,
...
export default async function AccountDeactivationsPage({ params, searchParams }: {
  ...
  const fields = ACCOUNT_DEACTIVATION_FILTER_FIELDS.map((f) => (
  ...
  return (
    <Page title="Account Deactivation" crumb="Deactivating existing non-default savings accounts, from request through to approval" user={user}>
      ...
      <Toolbar>
        ...
        {canCreate ? (
          <NewAccountDeactivationButton members={members} presetMemberId={presetMember ? Number(presetMember) : null} />
        ) : null}
      </Toolbar>
      ...
      <Card>
          ...
          <TableWrap>
            ...
            <tbody>
              {requests.map((r) => {
                ...
                return (
                <tr key={r.no}>
                  <td className="mono"><Link href={`/account-deactivations/view/${r.no}?view=${tab}`}>{r.no}</Link></td>
                  <td>
        ...
        ) : <EmptyState icon="🚫" title="No deactivation requests here" sub="Try a different tab or clear the search" />}
      </Card>
    </Page>
  );
}
