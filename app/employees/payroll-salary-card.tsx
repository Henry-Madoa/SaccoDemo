...
export interface PayrollSalaryValues {
  /** SACCO Member No. — net salary goes to this member's FOSA account; their checkoff loans are deducted. */
...
export interface PayrollSalaryLookups {
  ...
  members: { id: number; member_no: string; first_name: string; last_name: string }[];
  /** The member's Withdrawable Deposit (FOSA) account, resolved server-side — or null. */
  fosaAccount?: { account_no: string } | null;
...
export function PayrollSalaryCard({ values: v, basicPay, cumulative, lookups, canEdit, onSave, title = 'Payroll details', sub }: {
  ...
  const [memberId, setMemberId] = useState(String(v.member_id ?? ''));
    ...
    <CollapsibleCard title={title} sub={sub ?? 'Payroll Salary Card — posting group, payment mode, statutory deductions and pay status'}
      actions={canEdit && !editing ? (
        <button type="button" className="btn sm ghost"
          onClick={() => { setPostingGroupId(String(v.posting_group_id ?? '')); setMemberId(String(v.member_id ?? '')); setGradeId(String(v.job_grade_id ?? '')); setScaleId(String(v.salary_scale_id ?? '')); setBasicPayRaw(((basicPay ?? v.basic_pay_cents) / 100).toFixed(2)); setBasicFromScale(false); setCurrency(v.payroll_currency_code ?? ''); setSuspend(v.suspend_pay); setEditing(true); }}>
          Edit
        </button>
      ) : null}
    >
        ...
        <div className="grid g2 dl-groups">
          <section className="dl-group">
            ...
            <DefinitionList items={[
              ...
              ['Payment mode', <span key="pm">{v.payment_mode}{v.payment_mode === 'FOSA' ? <span className="tiny"> · net pay credited to the FOSA account</span> : null}</span>],
              ['Currency', v.payroll_currency_code || 'Base currency'],
              ['SACCO Member No.', v.member_no ? <><span className="mono">{v.member_no}</span> <span className="tiny">{v.member_name}</span></> : <span className="tiny muted-cell">Not a member</span>],
              ['FOSA account', lookups.fosaAccount ? <span className="mono" key="fa">{lookups.fosaAccount.account_no}</span> : (v.member_id ? <Pill tone="warn" key="fa">No withdrawable deposit account</Pill> : '—')],
            ]} />
          </section>
          <section className="dl-group">
        ...
        <>
          <form ref={formRef} onSubmit={(ev) => ev.preventDefault()}>
            ...
            <div className="grid g2">
              <SearchableSelect id="f_member_id" name="member_id" label="SACCO Member No."
                items={lookups.members} getValue={(m) => String(m.id)} getLabel={(m) => `${m.member_no} — ${m.first_name} ${m.last_name}`}
                value={memberId} onChange={setMemberId} placeholder="Search member…" emptyText="No matching members"
                hint="Net salary is paid into this member's FOSA (withdrawable deposit) account when the payment mode is FOSA, and their check-off loans are deducted on every payroll run." />
            </div>
            <div className="grid g3">
