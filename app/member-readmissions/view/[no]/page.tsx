...
import {
  getMemberReadmissionRequest, getAdjacentMemberReadmissionNos, getMemberReadmissionJournal,
...
export default async function MemberReadmissionDetailPage({ params, searchParams }: {
  ...
  const view = MEMBER_READMISSION_VIEWS.includes(viewRaw as MemberReadmissionView) ? (viewRaw as MemberReadmissionView) : undefined;
  const request = await getMemberReadmissionRequest(no);
  if (!request) notFound();
  ...
  const [canCreate, canApprove, tasks, { prevNo, nextNo }, journal] = await Promise.all([
    ...
    listWorkflowTasksForDocument('MEMBER_READMISSION', no),
    getAdjacentMemberReadmissionNos(no, view),
    request.status === 'Processed' ? getMemberReadmissionJournal(no) : Promise.resolve(null),
  ]);
  ...
  const routedTask = request.status === 'Pending Approval'
    ? await findPendingRoutedTask('MEMBER_READMISSION', no)
    : null;
  ...
  return (
    <>
      <CardNav
        prevHref={prevNo ? `/member-readmissions/view/${prevNo}${view ? `?view=${view}` : ''}` : null}
        nextHref={nextNo ? `/member-readmissions/view/${nextNo}${view ? `?view=${view}` : ''}` : null}
      />
      <Page
      ...
      <Card>
        ...
        <DefinitionList items={[
          ...
          request.transaction_charge_id ? ['Re-admission charge', `${request.transaction_charge_code} — ${request.transaction_charge_description}`] : null,
      ...
      <Card>
        ...
        <DefinitionList items={[
          ...
          ['Processed', processed
            ? <Pill tone="ok" key="processed">YES — member re-admitted</Pill>
      ...
      {processed ? (
        <Card>
          ...
          ) : <EmptyState icon="⚖" title="No re-admission charge was posted" sub="Re-admission was free — no charge was configured for this request" />}
        </Card>
      ) : null}
      </Page>
    </>
