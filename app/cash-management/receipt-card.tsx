 ...
 */
export function ReceiptCard({ receipt, lookups, canEdit, memberNos = new Map() }: {
  receipt: ReceiptDetail; lookups: ReceiptFormProps | null; canEdit: boolean;
  /** Member no. by id, for the lines that name a member other than the header's. */
  memberNos?: Map<number, string>;
}) {
  ...
  const isMember = receipt.receipt_type === 'Member';
        ...
        <>
          ...
          {receipt.lines.length ? (
            <TableWrap>
              <thead>
                <tr>
                  {isMember ? <th style={{ width: '18%' }}>Member</th> : null}
                  <th style={{ width: '24%' }}>{isMember ? 'Paid into' : 'Account'}</th>
                  <th>Description</th>
                  <th style={{ width: '20%' }}>{isMember ? 'Was owing' : 'Applies to Doc. No.'}</th>
              ...
              <tbody>
                {receipt.lines.map((l) => (
                  <tr key={l.id}>
                    {isMember ? (
                      <td>
                        {l.member_id && l.member_id !== receipt.member_id
                          ? <span className="mono">{memberNos.get(l.member_id) ?? l.member_id}</span>
                          : <span className="muted-cell">{receipt.member_no}</span>}
                      </td>
                    ) : null}
                    {/* A member line names the account or loan it credits; every other type carries
                        a code, and the name stacks under it because a G/L name is long. */}
                    <td>
                    ...
                    <td>
                      {isMember
                        ? (l.loan_id
                          ? (<><Money cents={l.loan_balance} /><div className="tiny muted-cell">before this payment</div></>)
                          : <span className="muted-cell">— deposit</span>)
                        : (l.applies_to_doc_no
                    ...
                    <td className="num">
                      ...
                      {l.charge_amount
                        ? <div className="tiny muted-cell">less charge <Money cents={l.charge_amount} /></div>
                        : null}
                    </td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr><td colSpan={isMember ? 4 : 3}>Total</td><td className="num"><b><Money cents={receipt.amount} /></b></td></tr>
              </tfoot>
            </TableWrap>
          ) : (
