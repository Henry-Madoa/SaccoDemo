 ...
 * voucher that actually carries tax, and a member payment shows what was drawable instead.
 */
export function PaymentVoucherCard({ pv, lookups, canEdit }: {
  ...
  const isMember = pv.pv_type === 'Member Payment' || pv.pv_type === 'RTGS/SWIFT';
  ...
  const showTax = !isMember && (vatTotal > 0 || whtTotal > 0);
        ...
        <>
          <div className="grid g2">
            ...
            <DefinitionList items={[
              ...
              ...(showTax
                ...
                : []),
              [isMember ? 'Paid to member' : 'Net paid', <Money cents={pv.total_amount} key="n" />],
          ...
          {pv.lines.length ? (
            <TableWrap>
              <thead>
                <tr>
                  <th style={{ width: '24%' }}>{isMember ? 'Paid from' : 'Account'}</th>
                  <th>Description</th>
                  <th style={{ width: '18%' }}>{isMember ? 'Available' : 'Applies to Doc. No.'}</th>
              ...
              <tbody>
                {pv.lines.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td>
                      {isMember
                        ? <Money cents={l.available_balance} />
                        : (l.applies_to_doc_no
