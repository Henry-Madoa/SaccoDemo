...
import { saveMemberEditBiometric, type BiometricKind } from '@/app/actions/media';
...
export interface EditRequestBiometricPanelProps {
  editNo: string;
...
/** Front/back ID scan, signature and fingerprint capture — tabbed, one slot each, staged on the edit request. */
export function EditRequestBiometricPanel({
  editNo, images, canEdit, mediaEnabled,
}: EditRequestBiometricPanelProps) {
...
function BiometricSlot({ editNo, kind, label, src, canEdit, mediaEnabled }: {
  editNo: string; kind: BiometricKind; label: string; src: string | null;
  ...
  const apply = async (file: UploadedFile | null) => {
    setBusy(true);
    try {
      const res = await saveMemberEditBiometric(editNo, kind, file);
