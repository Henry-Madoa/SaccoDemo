    ...
    <>
      ...
      {open ? (
        <FormModal
          ...
          onSubmit={async () => {
            const res = await saveEditNominees(
              editNo,
...
export function EditNomineePanel({ editNo, nominees, canManage, onSaved }: {
  editNo: string;
  nominees: MemberEditNominee[];
  canManage: boolean;
  onSaved?: () => void;
  ...
  return (
    <Card>
      <CardHead title="Nominees & beneficiaries" sub="Shares of benefit on the member's account">
        {canManage ? (
          <EditNomineeFormButton editNo={editNo} nominees={nominees} className="btn sm ghost" onSaved={onSaved}>
            Manage
          </EditNomineeFormButton>
        ) : null}
      </CardHead>
      {nominees.length ? (
        <TableWrap>
...
export function EditNextOfKinPanel({ editNo, nextOfKin, canManage, onSaved }: {
  editNo: string;
  nextOfKin: MemberEditNextOfKin[];
  canManage: boolean;
  onSaved?: () => void;
  ...
  return (
    <Card>
      <CardHead title="Next of kin" sub="A member can have more than one">
        {canManage ? (
          <EditNextOfKinFormButton editNo={editNo} nextOfKin={nextOfKin} className="btn sm ghost" onSaved={onSaved}>
            Manage
          </EditNextOfKinFormButton>
        ) : null}
      </CardHead>
      {nextOfKin.length ? (
        <TableWrap>
