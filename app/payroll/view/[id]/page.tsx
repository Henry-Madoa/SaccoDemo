...
import {
  getOpenPayrollPeriod, listEmployeeTransactions, listPeriodTransactions, getPayrollCumulatives, getMemberFosaAccount,
  listEmployeePayrollHistory, listPayrollLoansForEmployee,
} from '@/lib/payroll';
 ...
 * registrations, the bank / FOSA destination, this period's lines and computed payslip, the
 * check-off loans the run deducts, imprest recoveries, and the payroll history with cumulatives.
...
export default async function EmployeePayrollPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [contract, period, canRun, canManage, codes, captions, bankAccounts, cumulative, history, fosa, loans, postingGroup, imprest, scale] = await Promise.all([
    ...
    emp.member_id ? getMemberFosaAccount(emp.member_id) : Promise.resolve(undefined),
    emp.member_id ? listPayrollLoansForEmployee(emp.member_id) : Promise.resolve([]),
  ...
  const loanDeductionRows = transactions.filter((t) => t.loan_id);
  ...
  return (
    <Page title={`${emp.first_name} ${emp.last_name} — Payroll`} crumb={`${emp.employee_no}${period ? ` · ${period.period_name}` : ' · No open period'}`} user={user}>
      ...
      <Card>
        <div className="biometric-slot" style={{ alignItems: 'center', gap: 16 }}>
          ...
          <div className="inline" style={{ gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            {emp.payment_mode === 'FOSA' && !fosa ? <Pill tone="warn">No FOSA account</Pill> : null}
          </div>
        </div>
      ...
      {thisRun ? (
        <div className="grid g4 stack-2">
          ...
          <KpiTile label="Net pay" value={<Money cents={thisRun.net} />} accent
            foot={<span className="tiny">Paid by {emp.payment_mode}{emp.payment_mode === 'FOSA' && fosa ? ` → ${fosa.account_no}` : ''} · employer cost +{(thisRun.employer / 100).toLocaleString()}</span>} />
        </div>
      ) : null}
      ...
      <div className="grid g2">
        ...
        <div>
          ...
          <CollapsibleCard title="Pay destination" sub="Where the net pay goes">
            <DefinitionList items={[
              ['SACCO Member No.', emp.member_no ? <><span className="mono">{emp.member_no}</span> <span className="tiny">{emp.member_name}</span></> : <span className="tiny muted-cell">Not a member</span>],
              ['FOSA account', fosa ? <span className="mono" key="f">{fosa.account_no}</span> : (emp.member_id ? <Pill tone="warn" key="f">No withdrawable deposit account</Pill> : '—')],
          ...
          <Card>
            <CardHead title="Earnings & deductions" sub={`Recurring and one-off lines for ${period.period_name}`}>
              {canManage && period.status === 'OPEN' ? <AddTransactionButton employeeId={employeeId} periodId={period.id} periodStart={period.start_date}
                codes={codes.filter((c) => !transactions.some((t) => t.transaction_code_id === c.id && !t.loan_id))} /> : null}
            </CardHead>
            {transactions.length ? (
              <TableWrap>
                ...
                <tbody>
                  {transactions.map((t) => (
                    <tr key={t.id}>
                      <td>{t.transaction_code_name}{t.loan_id ? <> <Pill tone="info">Credit module</Pill></> : null}{t.salary_scale_id ? <> <Pill tone="accent">From scale</Pill></> : null}</td>
                      ...
                      <td className="num">
                        {canManage && period.status === 'OPEN' && !t.loan_id ? <>
                ...
                <tbody>
                  {periodLines.map((l) => (
                    <tr key={l.id} className={l.transaction_type === 'MEMO' ? 'muted-row' : undefined}>
                      ...
                      <td>{l.transaction_name}{l.transaction_type === 'MEMO' ? <> <Pill tone="">Working</Pill></> : null}{l.loan_id ? <> <Pill tone="info">Credit module</Pill></> : null}</td>
      ...
      <div className="grid g2">
        <CollapsibleCard title="SACCO loans recovered through payroll" sub="The member's check-off loans — deducted on every run, repaid in Credit when the period posts">
          {loans.length ? (
            <TableWrap>
              <thead><tr><th>Loan</th><th>Product</th><th className="num">Installment</th><th className="num">Balance</th><th className="num">This period</th></tr></thead>
              <tbody>
                {loans.map((l) => {
                  const row = loanDeductionRows.find((t) => t.loan_id === l.id);
                  return (
                    <tr key={l.id}>
                      <td className="mono"><Link href={`/loans/view/${l.id}`}>{l.loan_no}</Link></td>
                      <td>{l.product_name}</td>
                      <td className="num"><Money cents={l.installment} /></td>
                      <td className="num"><Money cents={l.owed} /></td>
                      <td className="num">{row ? <Money cents={row.amount_cents} /> : <span className="tiny muted-cell">Run to compute</span>}</td>
                    </tr>
                  );
                })}
              </tbody>
            </TableWrap>
          ) : <EmptyState icon="🏦" title={emp.member_id ? 'No check-off loans' : 'Not a SACCO member'} sub={emp.member_id ? 'Loans with recovery mode Check-off appear here once disbursed.' : 'Link a Member No. on the Payroll Salary Card to recover loans through payroll.'} />}
        </CollapsibleCard>
